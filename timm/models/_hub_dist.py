...
def load_custom_from_hf(model_id: str, filename: str, model: torch.nn.Module):
    ...
    hf_model_id, hf_revision = hf_split(model_id)
    cached_file = hf_hub_download(hf_model_id, filename=filename, revision=hf_revision)
    return model.module.load_pretrained(cached_file)
...
def save_config_for_hf(
    ...
    hf_config = {}
    pretrained_cfg = filter_pretrained_cfg(model.module.pretrained_cfg, remove_source=True, remove_null=True)
    # set some values at root config level
    hf_config['architecture'] = pretrained_cfg.pop('architecture')
    hf_config['num_classes'] = model_config.pop('num_classes', model.module.num_classes)

    # NOTE these attr saved for informational purposes, do not impact model build
    hf_config['num_features'] = model_config.pop('num_features', model.module.num_features)
...
def save_for_hf(
    ...
    tensors = model.module.state_dict()
